...
##### Create Model 2 - a model with missings addressed and all of the transformed and added variables.  We also take the log of TARGET_AMT.  We find the model with the highest AIC using stepAIC from the MASS package in R.  
...
df8AMT$TARGET_AMT = log(df8AMT$TARGET_AMT+1)
zz <- EHModel_Regression_StandardLM(df8AMT, "TARGET_AMT", splitRatio = 1, xseed=10, tests = FALSE)
print(hist(df8AMT$TARGET_AMT))
hist(zz$residuals)

```

The adjusted-R-Squared is very low, despite the number of significant variables and significance overall. The distribution of the residuals imporves but the tails are still an issue.  There still appear to be a large number of outliers.
...
qwx <- EHModel_Regression_StandardLM(df8AMT, "TARGET_AMT", splitRatio=1, xseed=10, tests = FALSE, returnLM = TRUE)
qwy <- EHModel_Regression_Robust(df8AMT, "TARGET_AMT")
...
tab <- matrix(c(4584, .86, .035, .239, 221, .807), ncol=2, byrow=TRUE)
...
It should be noted that the refined model sigma is not comparable to the base model because we have taken the log of the dependent variable.  The refined model clearly outperforms the base model. While the robust model outperforms the base standard model by a wide margin, the improvement in the 
