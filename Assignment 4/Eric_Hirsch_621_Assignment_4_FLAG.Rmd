...
The refined model has an AIC of 7587.  This is not an improvment. 
...
aa <- EHModel_Regression_StandardLM(dfNum_AMT, "TARGET_AMT", tests=FALSE, splitRatio=1, xseed=10)
hist(aa$residuals)
...
##### Create Model 2 - a model with missing values addressed and all of the transformed and added variables included.  We also take the log of TARGET_AMT.  We find the model with the highest AIC using stepAIC from the MASS package in R.  
...
tab <- matrix(c(7605, .80, .035, .239, 2334, .573), ncol=2, byrow=TRUE)
