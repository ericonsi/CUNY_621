...
dfc6 <- dfc5 %>%
  mutate(MVR_PTS_NA = ifelse(is.na(MVR_PTS),0,1), YOJ_NA = ifelse(is.na(YOJ),0,1), INCOME_NA = ifelse(is.na(INCOME),0,1), CAR_AGE_NA = ifelse(is.na(CAR_AGE),0,1), HOME_VAL_NA = ifelse(is.na(HOME_VAL),0,1))
...
dfMissing <- dfc6 %>%
  dplyr::select(TARGET_AMT, MVR_PTS_NA, YOJ_NA, INCOME_NA, CAR_AGE_NA, HOME_VAL_NA)

ehe

EHExplore_OneContinuousAndOneCategoricalColumn_Boxplots(dfMissing, "TARGET_AMT")

#dfc6 <- EHPrepare_MissingValues_Imputation(dfc5, "TARGET_AMT")
...
```{r}
library(MASS)
EHModel_Regression_StandardLM(dfc5, "TARGET_AMT")
