...
dfc7a <- EHPrepare_MissingValues_Imputation(dfc5, "TARGET_AMT")
...
#3. Log of y - still nonormal (but closer to normal) with outliers, but now homoskedatsic
...
dfc73 <- na.omit(dfc73)
zz <- EHModel_Regression_StandardLM(dfc73, "TARGET_AMT")
...
library(MASS)
  ...
  print(summary(step3))
    ...
    Y_test<- test_reg[,y]
    dfTest <- cbind(test_reg, pred_linreg)
    
    dfTest <- dfTest %>% 
    dplyr::select(-TARGET_AMT)
    
    q <- lm(TARGET_AMT ~ ., test_reg)
    print(summary(q))
...
robust1 <- rlm(TARGET_AMT ~ ., data=zz$model)
robust2 <- rlm(TARGET_AMT ~ ., data=aa$model)
robust3 <- rlm(TARGET_AMT ~ ., data=cc$model)

ols1 <- lm(TARGET_AMT ~ ., data=zz$model)
ols2 <- lm(TARGET_AMT~., data=aa$model )
ols3 <- lm(TARGET_AMT~., data=cc$model )
