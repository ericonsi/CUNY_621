...
df <- read.csv("C:\\Users\\eric.hirsch\\Desktop\\Rstudio\\CUNY_621\\Assignment 3\\crime-training-data_modified.csv", header=TRUE)
#dfEval <- read.csv("D:\\RStudio\\CUNY_621\\Assignment 3\\crime-training-data_modified.csv")
dfEval <- read.csv("C:\\Users\\eric.hirsch\\Desktop\\Rstudio\\CUNY_621\\Assignment 3\\crime-training-data_modified.csv", header=TRUE)

#df2 <- read.csv("D:\\RStudio\\CUNY_621\\Assignment 4\\insurance_training_data.csv")
df2 <- read.csv("C:\\Users\\eric.hirsch\\Desktop\\Rstudio\\CUNY_621\\Assignment 4\\insurance_training_data.csv", header=TRUE)
...
a <- EHSummarize_StandardPlots(df2Crashed, "TARGET_AMT", type="scatter")
a <- EHSummarize_StandardPlots(df2, "TARGET_FLAG", type="box")
```
...
```{r}
library(MASS)
df5Cr <- EHPrepare_MissingValues_Imputation(df5Cr, "TARGET_AMT")

df5Cr <- df5Cr %>% 
   dplyr::filter(rownames(df5Cr) !="411" & rownames(df5Cr) !="126" & rownames(df5Cr) !="29"  & rownames(df5Cr) !="319" & rownames(df5Cr) !="52")

df5Cr$TARGET_AMT = log(df5Cr$TARGET_AMT)

EHModel_Regression_StandardLM(df5Cr, "TARGET_AMT")
