...
df6 <- df %>%
  mutate(STARS_NA = ifelse(is.na(STARS),0,1), Sulphates_NA = ifelse(is.na(Sulphates),0,1), TotalSulpherDioxide_NA = ifelse(is.na(TotalSulfurDioxide),0,1), Alcohol_NA = ifelse(is.na(Alcohol),0,1), FreeSulfurDioxide_NA = ifelse(is.na(FreeSulfurDioxide),0,1), Chlorides_NA = ifelse(is.na(Chlorides),0,1), ResidualSugar_NA = ifelse(is.na(ResidualSugar),0,1), pH_NA = ifelse(is.na(pH),0,1))
...
write.csv(df7, "D:\\RStudio\\CUNY_621\\Assignment 5\\dataset.csv")
...
df7a <- EHPrepare_ScaleAllButTarget(df7, "TARGET")
a <- EHModel_Regression_StandardLM(df7a, "TARGET", splitRatio=1, tests = FALSE, xseed=10)
