...
```{r}
EHModel_Regression_Logistic(df, "target", splitRatio = .99)
...
The base model alone is an excellent predictor of crime rate. Only 4 of the 12 predictors are not significant and accuracy is over 91%. AIC is 218.

When we remove the non-significant variables the prediction stays at 91% but the AIC falls slightly to 215.


Now we try a model with all of ourdummy variables from above:
...
z <- EHModel_Regression_Logistic(dfInt2, "target", splitRatio = .99 )
```
...
Our AIC is actually lower (AIC=178), and the model does a better job predicting outcomes (Accuracy = .95).  Now we'll perform backward elimination to see if we can reduce AIC even further to avoid the possibility of overfitting.

```{r include = FALSE}

dfInt4 <- dfInt2 %>%
  dplyr::select(-NoxOverPoint8, -nox)

EHModel_Regression_Logistic(dfInt4, "target")

```
Our best model ultimately is below (the tax categories are not significant but nonetheless contribute to higher accuracy and lower AIC.)  The final results for this model averaging 100 rounds on an 80/20 split are 95.5% accuracy and an AIC of 162.7.


```{r}
dfInt5 <- dfInt4 %>%
  dplyr::select(-radOver10, -ZnOver0, -lstat_log, -lstat, -zn, -indus, -MedvBelow50, -chas, -medv )

dfInt5 <- dfInt5 %>%
  dplyr::filter(rownames(dfInt5) != 338)

z <- EHModel_Regression_Logistic(dfInt5, "target", splitRatio = .99 )
```
```{r  include=FALSE}
...
EHModel_Regression_Logistic(dfInt6, "target", splitRatio = .99)
...
```{r}
dfSmall <- dfInt5 %>%
  dplyr::select(dis_log, age_log, rmSquared, ptratio, dis, age , rm, target)

EHModel_Regression_Logistic(dfSmall, "target")
```
...
for (i in 1:100)
{
  q <- EHModel_Regression_Logistic(dfSmall, "target")
  acc[i]=q[2]
  AIC[i]=q[3]
}
