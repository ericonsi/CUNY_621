...
for (i in 1:10)
{
  a <- EHModel_Regression_Logistic(df2, "target")
}
...
df1 <- df %>%
  dplyr::select(nox, ptratio, rm, target)
...
library(stats)
interaction.plot(dfT$TaxOver600, df$target, df$indus)
...
for (i in 1:10)
{
  a <- EHModel_Regression_Logistic(df5, "target")
}
