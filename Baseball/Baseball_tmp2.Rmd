...
summary(lm(TARGET_WINS ~ Hitting_Power + Pitching_Weakness, dfCat))
```
The model shows that in the balance between hitting and pitching, __*teams should emphasize good hitting and accept weak pitching.*__  The adjusted r squared for the model with Hitting Power alone (.1573) is improved very little when pitching weakness is added to it (.16).  The r-suqred for Pitching Weakness alone is .06.

### Select a Model and Make Predicitons

Now we make predictions.  The second model has the highest R squared and reliable interpretability so we will use it for our predictions.  We will first eliminate the few influential pints indicated by the residual plots.
