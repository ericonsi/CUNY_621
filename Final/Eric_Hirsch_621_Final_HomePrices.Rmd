...
home_submit <- read.csv("C:\\Users\\eric.hirsch\\Desktop\\RStudio\\605_Final\\Final\\housing\\sample_submission.csv", header = TRUE)
```
...
# The na probably stands more for "not applicable" than missing, but we remove because these categories don't seem so important.  Actually we don't becuase they may be missing in the test set.  So we impute the mesn.
home_sales1 <- home_sales %>%
  dplyr::select(-FireplaceQu, -Fence, -Alley, -MiscFeature, -PoolQC, -LotFrontage)

home_test1 <- home_kaggle_test %>%
  dplyr::select(-FireplaceQu, -Fence, -Alley, -MiscFeature, -PoolQC, -LotFrontage)

EHSummarize_MissingValues(home_sales1)

home_sales2 <- EHPrepare_MissingValues_Imputation(home_sales1)
home_test2 <- EHPrepare_MissingValues_Imputation(home_test1)
...
df5 <- EHPrepare_CreateDummies(home_sales2)
dfSubmit5 <- EHPrepare_CreateDummies(home_test2)

```


We Check if the dataset is missing columns from the evaluation dataset and if, so drop them from the original
```{r}
g <- EHPrepare_RestrictDataFrameColumnsToThoseInCommon(df5, dfSubmit5, exclude=c("SalePrice"))
...
mult6 <- EHExplore_Multicollinearity(df5, printHighest=TRUE, threshold=.8, printHeatMap=FALSE)
...
library(caTools)
library(Metrics)
df6$SalePrice <- log(df6$SalePrice)

dfSubmit6 <- dfSubmit5R

dfSmall <- df6 %>%
  dplyr::select(SalePrice, OverallQual, OverallCond, YearBuilt, LotArea, GrLivArea, TotRmsAbvGrd)
```
...
#abc <- EHModel_Regression_StandardLM(dfSmall, "SalePrice", splitRatio = 1, returnLM = TRUE)

abc <- EHModel_Regression_StandardLM(df7, "SalePrice", splitRatio = 1, returnLM=TRUE)
...
```{r}
write_csv(x1, "C:\\Users\\eric.hirsch\\Desktop\\RStudio\\CUNY_621\\Final\\coeffs.csv")
x1
...
dfLess <- df7 %>%
  dplyr::select(-Id, -LandContour_Low, -Neighborhood_BrDale, -Neighborhood_ClearCr, -Neighborhood_SWISU, -HouseStyle_SLvl, -Exterior1st_CemntBd, -Exterior2nd_CmentBd, -MasVnrType_NA, -SaleType_CWD, -SaleType_Con, -SaleCondition_Partial)
...
abc_less <- EHModel_Regression_StandardLM(dfLess, "SalePrice", splitRatio = 1, returnLM=TRUE)
...
df7a <- EHData::EHPrepare_ScaleAllButTarget(df7, "SalePrice")
dfSubmit7a <- as.data.frame(scale(dfSubmit7))
...
x3 <- EHPrepare_RestrictDataFrameColumnsToThoseInCommon(x1, x2)
...
{
predictions <- predict(df,newdata=dfSubmit7)
...
#write_csv(predictions, "D:\\RStudio\\CUNY_621\\Final\\predictions2.csv")
}

makePredictions2(abc_less)
...
predictions[is.na(predictions)] <- log(mean(home_sales$SalePrice))
predictions$SalePrice <- exp(predictions$SalePrice)
#write_csv(predictions, "C:\\Users\\eric.hirsch\\Desktop\\predictions2.csv")
...
dfN <- df5R %>%
    dplyr::select(SalePrice, starts_with('Neighborhood'))

EHModel_Regression_StandardLM(dfN, "SalePrice", splitRatio = 1)
...
```{r}
library(ggsci)
qs <- EHExplore_OneContinuousAndOneCategoricalColumn_Boxplots(dfN, "SalePrice", yCategorical = FALSE)

qs[[5]]

#grid.arrange(grobs = qs[c(1:16)], ncol=4)
#grid.arrange(grobs = qs[c(17:25)], ncol=4)
...
```{r}
#EHSummarize_StandardPlots(abc$model, "SalePrice")
...
makePredictions2 <- function(df)
{
predictions <- predict(df,newdata=dfSubmit5)
predictions <- data.frame(as.vector(predictions))
predictions$Id <- dfTest_Final$Id
predictions[,c(1,2)] <- predictions[,c(2,1)]
colnames(predictions) <- c("Id", "SalePrice")
