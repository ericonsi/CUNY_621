...
```{r}
df7 <- df6 %>%
  dplyr::mutate(BuiltAfter1920 = ifelse(YearBuilt>1920,1,0), YearRemodUnknown = ifelse(YearRemodAdd==1950,1,0), NoFinBsmt = ifelse(BsmtFinSF1==0,1,0)) %>%
  dplyr::filter(LotArea>3000)
```

```{r}
#abc <- EHModel_Regression_StandardLM(dfSmall, "SalePrice", splitRatio = 1, returnLM = TRUE)

abc <- EHModel_Regression_StandardLM(df6, "SalePrice", splitRatio = 1, returnLM=TRUE)
...
varmtx <- model.matrix(SalePrice~.-1, data=dfSmall)
response <- dfSmall$SalePrice
...
#y.pred <- as.matrix(cbind(const=1,dfSmall)) %*% coef(ridge)
